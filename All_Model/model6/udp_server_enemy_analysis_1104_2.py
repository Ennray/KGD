...
class EnemyAnalysisServer:
    ...
    def __init__(self, host="0.0.0.0", port=5005, websocket_port=7070):
        ...
        self.goals = [
            {"name": "港口", "lat": 28.3777972, "lon": 121.6020917},
            {"name": "海上风电厂", "lat": 27.400992, "lon": 121.367706},
    ...
    def detect_enemy1_turn_maneuver(self):
        """根据绝对航向范围判断转向"""
        with self._enemy1_lock:
            if len(self.enemy1_heading_history) < 1:
                return ""

            current_heading = list(self.enemy1_heading_history)[-1]

            print(f"🔄 当前航向: {current_heading:.1f}°")

            # 根据航向范围判断转向
            if 265 <= current_heading <= 275:
                # 265-275度之间：不输出转向
                print(f"📊 航向{current_heading:.1f}°在265-275°范围内，直线飞行")
                return ""
            elif 326 <= current_heading <= 335:
                # 326-335度：右转
                print(f"🎯 航向{current_heading:.1f}°在326-335°范围内，向右机动")
                return "right_turn"
            elif 34 <= current_heading <= 36:
                # 34-36度：右转
                print(f"🎯 航向{current_heading:.1f}°在34-36°范围内，向右机动")
                return "right_turn"
            elif 336 <= current_heading <= 340:
                # 336-340度：左转
                print(f"🎯 航向{current_heading:.1f}°在336-340°范围内，向左机动")
                return "left_turn"
            else:
                # 其他航向范围：不输出转向
                print(f"📊 航向{current_heading:.1f}°不在指定范围内，直线飞行")
                return ""

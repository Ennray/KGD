...
# cluster_labels = cls.cluster_classify(track_dict, dt=1.0)
# for tid, lab in cluster_labels.items():
#     engine.cluster_maneuver[tid] = lab
    ...
    def _get_predictor(self, track_id):
        if track_id not in self.predictors:
            self.predictors[track_id] = LongHorizonHeadingPredictor(
                ...
                n_particles=self.cfg.get("n_particles", 1000),
                w_cost=self.cfg.get("w_cost", 0.02),
                beta_goal=self.cfg.get("beta_goal", 0.3),
                rng_seed=self.cfg.get("rng_seed", 0)
            )
